...
sidebarTitle: Parameters reference
---

When you trace LLM calls with LangSmith, you often want to [track costs](/langsmith/cost-tracking), compare model configurations, and filter traces by model type. LangSmith's native integrations (like [LangChain](/langsmith/trace-with-langchain) or the [OpenAI](/langsmith/trace-openai)/[Anthropic](/langsmith/trace-anthropic) wrappers) handle this automatically, but custom model wrappers and self-hosted models need a way to provide this information. That's where `ls_` metadata parameters come in.

These special metadata parameters (all prefixed with `ls_`) let you pass model configuration and identification information through the standard `metadata` field. Once set, LangSmith can automatically calculate costs, display model information in the UI, and enable filtering and analytics across your traces.
...
- **Filter and analyze traces** by model type, provider, or configuration settings
...
| [`ls_model_type`](#ls-model-type) | `"chat"` \| `"text"` | No | Type of model (chat or text completion) |
...
**When to use:**
When you want proper categorization in the [LangSmith UI](https://smith.langchain.com).
...
**Optional, work independently:** [`ls_model_type`](#ls-model-type), [`ls_temperature`](#ls-temperature), [`ls_max_tokens`](#ls-max-tokens), [`ls_stop`](#ls-stop)
