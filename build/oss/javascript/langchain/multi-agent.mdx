...
* **Custom output formatting**: Adjust or enrich the sub-agent's response in code before handing it back to the main agent.
  ...
  * This requires wrapping the result in a [`Command`](https://langchain-ai.github.io/langgraphjs/reference/classes/langgraph.Command.html) (or equivalent structure) so you can merge custom state with the sub-agent's response.
...
At the core, handoffs rely on [persistent state](/oss/javascript/langchain/short-term-memory) that survives across conversation turns:
...
1. **[Custom state schema](/oss/javascript/langchain/short-term-memory#customizing-agent-memory)** (@[`AgentState`]) with `current_stage` field tracks which agent is active
2. **[Tools return `Command` objects](/oss/javascript/langchain/short-term-memory#write-short-term-memory-from-tools)** ([`Command`](https://langchain-ai.github.io/langgraphjs/reference/classes/langgraph.Command.html)) to update both state and messages
...
4. **[Checkpointer](/oss/javascript/langchain/short-term-memory#usage)** (@[`InMemorySaver`]) persists state across conversation turns
...
You can implement message scoping using @[`@before_model`] [middleware that trims messages](/oss/javascript/langchain/short-term-memory#trim-messages) before each agent sees them. The example below shows how to implement message scoping using @[`@before_model`] middleware:
...
See [message trimming strategies](/oss/javascript/langchain/short-term-memory#common-patterns) for more techniques.

## Allowing users to navigate back

A key UX requirement is allowing users to correct mistakes in handoff workflows. Add @[`@tool`]s that return [`Command`](https://langchain-ai.github.io/langgraphjs/reference/classes/langgraph.Command.html) objects to let agents navigate backward:
...
5. **Allow backward navigation**: Provide @[`@tool`]s that return [`Command`](https://langchain-ai.github.io/langgraphjs/reference/classes/langgraph.Command.html) objects for users to correct mistakes and return to previous stages.
6. **Ensure valid message sequences**: Make sure message histories follow provider requirements (user → AI → tool alternation).
7. **Use checkpointers**: Always include a @[`InMemorySaver`] [checkpointer](/oss/javascript/langchain/short-term-memory#usage) to persist state across turns.
